...
class BatchTransformer : public InternalThread {
 ...
 public:
  ...
  int get_processed_full_size() {
    return this->processed_full_.size();
  }
  
  int get_processed_free_size() {
    return this->processed_free_.size();
  }

  int get_prefetch_free_size(int qid) {
    return this->prefetches_free_[qid]->size();
  }
  
  int get_prefetch_full_size(int qid) {
    return this->prefetches_full_[qid]->size();
  }
