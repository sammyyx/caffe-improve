...
BasePrefetchingDataLayer<Ftype, Btype>::BasePrefetchingDataLayer(const LayerParameter& param,
    ...
    : BaseDataLayer<Ftype, Btype>(param, threads(param)),
      ...
      iter0_(true),
      io_delay_timer(make_shared<Timer>()),
      pop_delay_timer(make_shared<Timer>()),
      push_delay_timer(make_shared<Timer>()) {
      //data_forward_delay_timer(make_shared<Timer>()) {
...
void BasePrefetchingDataLayer<Ftype, Btype>::InternalThreadEntryN(size_t thread_id) {
  ...
  try {
    while (!must_stop(thread_id)) {
      ...
      //pop_delay_timer->Start();
      //LOG_IF(INFO, "QUEUE_SIZE") << "prefetch_free:" << batch_transformer_->get_prefetch_free_size(qid)
      //  << "|" << "prefetch_full:" << batch_transformer_->get_prefetch_full_size(qid) << std::endl;

      shared_ptr<Batch> batch = batch_transformer_->prefetched_pop_free(qid);
      //float pop_lapse = pop_delay_timer->Seconds();
      //LOG_IF(INFO, "DATA_LAYER") << "qid:" << qid << " pop delay:" << pop_lapse << "s" << std::endl;

      CHECK_EQ((size_t) -1L, batch->id());
      //io_delay_timer->Start();
      load_batch(batch.get(), thread_id, qid);
      if (must_stop(thread_id)) {
        break;
      }
      //float io_lapse = io_delay_timer->Seconds();
      //LOG_IF(INFO, "DATA_LAYER") << "qid:" << qid << " io delay:" << io_lapse << "s" << std::endl;

      //push_delay_timer->Start();
      batch_transformer_->prefetched_push_full(qid, batch);
      //float push_lapse = push_delay_timer->Seconds();
      //LOG_IF(INFO, "DATA_LAYER") << "qid:" << qid << " push delay:" << push_lapse << "s" << std::endl;
...
void BasePrefetchingDataLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
  ...
  data_forward_delay_timer->Start();
  ...
  //float lapse = data_forward_delay_timer->Seconds();
  //LOG_IF(INFO, "DATA_LAYER") << "data forward delay:" << lapse << "s" << std::endl;
}
