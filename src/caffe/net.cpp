...
Net::Net(const NetParameter& param,
    ...
    : root_net_(root_net),
      ...
      //delay_timer(make_shared<Timer>()),
      //forward_delay_timer(make_shared<Timer>()),
      //backward_delay_timer(make_shared<Timer>())
  Init(param);
}
...
float Net::ForwardBackward(bool apply_update) {
  float loss;
  //forward_delay_timer->Start();
  //delay_timer->Start();
  Forward(&loss);
  //float forward_lapse = forward_delay_timer->Seconds();
  //LOG_IF(INFO, "forward process") << "forward delay:"
  //  << forward_lapse << "s" << std::endl;
  //backward_delay_timer->Start();
  Backward(apply_update);
  //float backward_lapse = backward_delay_timer->Seconds();
  //LOG_IF(INFO, "backward process") << "backward delay:"
  //  << backward_lapse << "s" << std::endl;
  //float lapse = delay_timer->Seconds();
  //LOG_IF(INFO, "ForwardBackward") << "forward and backward delay:"
  //  << lapse << "s" << std::endl;
  return loss;
}
